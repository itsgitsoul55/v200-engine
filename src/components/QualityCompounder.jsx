    ...
    <div className="p-4 space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Avg ROCE', val: stocks.length ? `${(stocks.reduce((a, b) => a + (b.roce ?? 0), 0) / stocks.length).toFixed(1)}%` : '—', color: 'text-blue-400' },
              ...
              {filtered.map(s => (
                <tr key={s.symbol} onClick={() => setSelected(selected?.symbol === s.symbol ? null : s)}
                  ...
                  <td className="py-2 px-2 text-right">
                    <span className="bg-green-900/40 text-green-300 px-2 py-0.5 rounded text-xs font-bold">{s.quality_score ?? '—'}%</span>
                  </td>
                  <td className="py-2 px-2 text-right text-slate-200">{s.roce != null ? `${s.roce.toFixed(1)}%` : '—'}</td>
                  <td className="py-2 px-2 text-right text-slate-200">{s.rev_cagr_3yr != null ? `${s.rev_cagr_3yr.toFixed(1)}%` : '—'}</td>
                  <td className="py-2 px-2 text-right text-slate-200">{s.promoter_pct != null ? `${s.promoter_pct.toFixed(1)}%` : '—'}</td>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {criteria.map(c => (
              <div key={c.key} className={`flex items-center gap-2 text-sm px-2 py-1.5 rounded ${selected[c.key] ? 'bg-green-900/30 text-green-300' : 'bg-red-900/30 text-red-300'}`}>
                <span>{selected[c.key] ? '✓' : '✗'}</span>
                <span>{c.label}</span>
              </div>
            ))}
          </div>
          <div className="grid grid-cols-2 gap-3 text-sm mt-2">
            <div><span className="text-slate-400">ROCE:</span> <span className="text-slate-200">{selected.roce?.toFixed(1) ?? '—'}%</span></div>
            <div><span className="text-slate-400">D/E:</span> <span className="text-slate-200">{selected.de_ratio ?? '—'}</span></div>
            <div><span className="text-slate-400">Rev CAGR:</span> <span className="text-slate-200">{selected.rev_cagr_3yr?.toFixed(1) ?? '—'}%</span></div>
            <div><span className="text-slate-400">Promoter:</span> <span className="text-slate-200">{selected.promoter_pct?.toFixed(1) ?? '—'}%</span></div>
            <div><span className="text-slate-400">Pledge:</span> <span className="text-slate-200">{selected.pledge_pct?.toFixed(1) ?? '—'}%</span></div>
            <div><span className="text-slate-400">Profit Yrs:</span> <span className="text-slate-200">{selected.profitable_yrs ?? '—'}/7</span></div>
          </div>
        </div>
      )}
    </div>
